...
class Face:
    ...
    def add_tree(self, frame):
        ...
        # , height=self.cfg.get('tree_w', '10')
        self.treef = frame
        # self.treef.geometry(self.cfg.get('tree_g'))
        ...
        hsb = ttk.Scrollbar(frame, command=self.tree.xview, orient='horizontal')
    ...
    def add_text(self, frame):
        frame.grid_columnconfigure(0, weight=1)
        frame.grid_rowconfigure(0, weight=1)
        # w, h = self.cfg.get('text_wh', ('80', '10'))
        # , width=w, height=h
        text = Text(frame, state='disabled', wrap='word',
                    font='Times 14')
        self.text = text
        # self.text.geometry(self.cfg.get('text_g'))
        ...
        # hsb = ttk.Scrollbar(frame, command=self.text.xview, orient='horizontal')
        # hsb.grid(column=0, row=1, sticky='ew')
        # self.text['xscrollcommand'] = lambda f, l: autoscroll(hsb, f ,l)
        self.text_curinfo = None
        text.tag_configure('h1', font='Times 16 bold', relief='raised')

# The currently available configuration options for tags are: "background", "bgstipple", "borderwidth", "elide", "fgstipple", "font", "foreground", "justify", "lmargin1", "lmargin2", "offset", "overstrike", "relief", "rmargin", "spacing1", "spacing2", "spacing3", "tabs", "tabstyle", "underline", and "wrap". 
    ...
    def add_menu(self):
        ...
        self.mfile.add_command(label='Select default folder...', command=self.ask_dir)
        ...
        if type(dname) == str and dname:
            if not isdir(dname):
                if messagebox.askyesno(
                    message='Are you sure you want to create a new directory?',
                    icon='question', title='New directory') == False:
                    return
                makedirs(dname)
            if text is not None:
    ...
    def del_page(self, evt=None):
        if type(evt) == str:
            ...
            if iid in self.remember:
                if messagebox.askyesno(
                    message='Are you sure you want to delete preserved page?',
                    icon='question', title='Install') == False:
                    return
